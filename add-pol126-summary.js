// add-pol126-summary.js
// Creates the Summary record for POL126 (Citizen and the State).
//
...
//        C:\Users\HP\Desktop\ubora-world\private-uploads\pol126-summary.pdf
//   2. Confirm POL126 has already been seeded (seed-pol126.js) AND that
//      you have already run crosslist-courses.js after seeding it.
//
// Run from the project root:
//   node add-pol126-summary.js
...
const PDF_PATH = path.join(process.cwd(), 'private-uploads', 'pol126-summary.pdf');

async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'POL126' } });
  if (!course) {
    console.error('POL126 course not found. Run seed-pol126.js first.');
  ...
  if (existing) {
    console.log('A summary already exists for POL126:', existing.id);
  ...
  if (!fs.existsSync(PDF_PATH)) {
    ...
    console.error('Place pol126-summary.pdf in the private-uploads folder and re-run.');
    await prisma.$disconnect();
    process.exit(1);
  }
  ...
  const summary = await prisma.summary.create({
    data: {
      courseId: course.id,
      title: 'POL126 Course Summary - Citizen and the State',
      topicCount: 22,
      pageCount: 5,
      fileUrl: 'db-stored:pol126-summary',
      fileData: buffer,
      status: 'draft',
    },
  });
