// add-pcr111-summary.js
// Creates the Summary record for PCR111 (Introduction to Peace Studies).
//
// SECURITY: same pattern as add-eco121-summary.js, add-gst107-summary.js,
// and add-css111-summary.js - the PDF is read from the non-public
...
// so PCR111's summary shows up in the /admin/content review queue
// before going live, same as its questions did.
//
// BEFORE RUNNING:
//   1. Place the finished PDF at:
//        C:\Users\HP\Desktop\ubora-world\private-uploads\pcr111-summary.pdf
//   2. Confirm PCR111 has already been seeded (seed-pcr111.js), since this
//      script only attaches a Summary to the existing Course row - it does
//      not create or modify the Course itself.
//
// Run from the project root:
//   node add-pcr111-summary.js
...
const PDF_PATH = path.join(process.cwd(), 'private-uploads', 'pcr111-summary.pdf');

async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'PCR111' } });
  if (!course) {
    console.error('PCR111 course not found. Run seed-pcr111.js first.');
  ...
  if (existing) {
    console.log('A summary already exists for PCR111:', existing.id);
  ...
  if (!fs.existsSync(PDF_PATH)) {
    ...
    console.error('Place pcr111-summary.pdf in the private-uploads folder and re-run.');
    await prisma.$disconnect();
    process.exit(1);
  }
  ...
  const summary = await prisma.summary.create({
    data: {
      courseId: course.id,
      title: 'PCR111 Course Summary - Introduction to Peace Studies',
      topicCount: 20,
      pageCount: 9,
      fileUrl: 'db-stored:pcr111-summary',
      fileData: buffer,
      status: 'draft',
    },
  });
