// add-css134-summary.js
// Creates the Summary record for CSS134 (Geography of Nigeria).
//
...
//        C:\Users\HP\Desktop\ubora-world\private-uploads\css134-summary.pdf
//   2. Confirm CSS134 has already been seeded (seed-css134.js) AND that
//      you have already run crosslist-courses.js after seeding it.
//
// Run from the project root:
//   node add-css134-summary.js
...
const PDF_PATH = path.join(process.cwd(), 'private-uploads', 'css134-summary.pdf');

async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'CSS134' } });
  if (!course) {
    console.error('CSS134 course not found. Run seed-css134.js first.');
  ...
  if (existing) {
    console.log('A summary already exists for CSS134:', existing.id);
  ...
  if (!fs.existsSync(PDF_PATH)) {
    ...
    console.error('Place css134-summary.pdf in the private-uploads folder and re-run.');
    await prisma.$disconnect();
    process.exit(1);
  }
  ...
  const summary = await prisma.summary.create({
    data: {
      courseId: course.id,
      title: 'CSS134 Course Summary - Geography of Nigeria',
      topicCount: 19,
      pageCount: 5,
      fileUrl: 'db-stored:css134-summary',
      fileData: buffer,
      status: 'draft',
    },
  });
