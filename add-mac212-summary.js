// add-mac212-summary.js
// Creates the Summary record for MAC212 (Media and Society).
//
...
//        C:\Users\HP\Desktop\ubora-world\private-uploads\mac212-summary.pdf
//   2. Confirm MAC212 has already been seeded (seed-mac212.js) AND that
//      you have already run crosslist-courses.js after seeding it.
//
// Run from the project root:
//   node add-mac212-summary.js
...
const PDF_PATH = path.join(process.cwd(), 'private-uploads', 'mac212-summary.pdf');

async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'MAC212' } });
  if (!course) {
    console.error('MAC212 course not found. Run seed-mac212.js first.');
  ...
  if (existing) {
    console.log('A summary already exists for MAC212:', existing.id);
  ...
  if (!fs.existsSync(PDF_PATH)) {
    ...
    console.error('Place mac212-summary.pdf in the private-uploads folder and re-run.');
    await prisma.$disconnect();
    process.exit(1);
  }
  ...
  const summary = await prisma.summary.create({
    data: {
      courseId: course.id,
      title: 'MAC212 Course Summary - Media and Society',
      topicCount: 21,
      pageCount: 5,
      fileUrl: 'db-stored:mac212-summary',
      fileData: buffer,
      status: 'draft',
    },
  });
