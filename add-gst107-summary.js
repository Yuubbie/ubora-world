...
// Creates the Summary record for GST107 (A Study Guide for the Distance
// Learner).
//
...
// so GST107's summary shows up in the /admin/content review queue
// before going live, same as its questions did.
//
// BEFORE RUNNING:
//   1. Place the finished PDF at:
//        C:\Users\HP\Desktop\ubora-world\private-uploads\gst107-summary.pdf
//   2. Confirm GST107 has already been seeded (seed-gst107.js), since this
...
const PDF_PATH = path.join(process.cwd(), 'private-uploads', 'gst107-summary.pdf');

async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'GST107' } });
  if (!course) {
    console.error('GST107 course not found. Run seed-gst107.js first.');
  ...
  if (existing) {
    console.log('A summary already exists for GST107:', existing.id);
  ...
  if (!fs.existsSync(PDF_PATH)) {
    ...
    console.error('Place gst107-summary.pdf in the private-uploads folder and re-run.');
    await prisma.$disconnect();
    process.exit(1);
  }
  ...
  const summary = await prisma.summary.create({
    data: {
      courseId: course.id,
      title: 'GST107 Course Summary - A Study Guide for the Distance Learner',
      topicCount: 16,
      pageCount: 14,
