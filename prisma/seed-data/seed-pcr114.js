 ...
 * seed-pcr114.js
 *
 * Seeds PCR114 (Introduction to Conflict Resolution Processes II) into
 * the database.
 *
 ...
 *
 * PCR114 is owned by Peace and Conflict Resolution but is a REQUIRED
 * course for Criminology and Security Studies students too (same
 * pattern as PCR111, POL111, ECO121, CIT104, POL126). After running
 * this script, you MUST run `node crosslist-courses.js` to link it
 * into Criminology as well - do not skip this step. This is the LAST
 * course needed to fully complete second-semester Criminology.
 *
 * This course has a real Module 1-6 structure in its official NOUN
 * course guide (each with 2-5 units within), so the CSV files are
 * organised by the course's own modules, not by unit. There are 6 CSV
 * files.
 *
 ...
 *   node prisma\seed-data\seed-pcr114.js
 *   node crosslist-courses.js --dry-run
 *   node crosslist-courses.js
 *
 * Place all 6 PCR114_CBT_Bank_Module1-6.csv files in the same folder
...
async function main() {
  console.log("=== PCR114 Seed Script: Starting ===\n");
  ...
  let course = await prisma.course.findUnique({ where: { code: "PCR114" } });
  if (!course) {
    course = await prisma.course.create({
      data: {
        code: "PCR114",
        title: "Introduction to Conflict Resolution Processes II",
        departmentId: department.id,
        level: 100,
        semester: "second",
      },
    });
    console.log("Created Course: PCR114 - Introduction to Conflict Resolution Processes II (100L, 2nd semester)");
  } else {
    console.log("Found existing Course: PCR114");
  }
  ...
  if (!questionBank) {
    ...
    console.log("Created new draft QuestionBank for PCR114");
  } else {
    console.log(`Found existing QuestionBank for PCR114 (status: ${questionBank.status}) - will add/update questions in it`);
  }

  const dir = __dirname;
  const csvFiles = fs
    .readdirSync(dir)
    .filter((f) => f.startsWith("PCR114_CBT_Bank_Module") && f.endsWith(".csv"));

  if (csvFiles.length === 0) {
    console.log("\nNo PCR114_CBT_Bank_Module*.csv files found next to this script.");
    await prisma.$disconnect();
    return;
  }
  ...
  console.log(`\nNEXT STEP: run 'node crosslist-courses.js' so PCR114 also appears`);
  ...
  console.log(`This is the LAST course needed to fully complete second-semester Criminology.`);
