 ...
 * seed-css246.js
 *
 * Seeds CSS246 (Legal and Social Framework of Private Security Services
 * in Nigeria) into the database.
 *
 ...
 *
 * This is the FIRST 200-level course built for Criminology - all
 * previous courses (CSS111/121/133/112/132/134, ECO121, PCR111/114,
 * CIT104, POL111/126) were 100L. Level is explicitly set to 200 below.
 *
 * This course is owned directly by Criminology (same as CSS111/121/133/
 * 112/132/134) - it is not a service course from another department.
 *
 ...
 * Criminology. Do not skip this step - this is what makes the course
 * actually show up in the app.
 *
 * NOTE - the official NOUN course guide lists 20 units across 4
 * modules (5 units per module), but Module 4's Unit 3 ("Relationship
 * between Police and Private Security Operatives in Nigeria") has no
 * extractable body content in the source PDF - only its title appears
 * in the table of contents. Module 4's CSV therefore draws from its
 * other 4 units only. This does not affect question quality or
 * grounding - every question here is still traceable to real content
 ...
 *   node prisma\seed-data\seed-css246.js
 *   node crosslist-courses.js --dry-run
 *   node crosslist-courses.js
 *
 * Place all 4 CSS246_CBT_Bank_Module1-4.csv files in the same folder
...
async function main() {
  console.log("=== CSS246 Seed Script: Starting ===\n");
  ...
  let course = await prisma.course.findUnique({ where: { code: "CSS246" } });
  if (!course) {
    course = await prisma.course.create({
      data: {
        code: "CSS246",
        title: "Legal and Social Framework of Private Security Services in Nigeria",
        departmentId: department.id,
        level: 200,
        semester: "second",
      },
    });
    console.log("Created Course: CSS246 - Legal and Social Framework of Private Security Services in Nigeria (200L, 2nd semester)");
  } else {
    console.log("Found existing Course: CSS246");
  }
  ...
  if (!questionBank) {
    ...
    console.log("Created new draft QuestionBank for CSS246");
  } else {
    console.log(`Found existing QuestionBank for CSS246 (status: ${questionBank.status}) - will add/update questions in it`);
  }

  const dir = __dirname;
  const csvFiles = fs
    .readdirSync(dir)
    .filter((f) => f.startsWith("CSS246_CBT_Bank_Module") && f.endsWith(".csv"));

  if (csvFiles.length === 0) {
    console.log("\nNo CSS246_CBT_Bank_Module*.csv files found next to this script.");
    await prisma.$disconnect();
    return;
  }
  ...
  console.log(`\nREMINDER: run 'node crosslist-courses.js' now, even though CSS246 is`);
  ...
  console.log(`\nThis is the FIRST 200L course built for Criminology.`);
