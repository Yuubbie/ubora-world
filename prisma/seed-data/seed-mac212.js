 ...
 * seed-mac212.js
 *
 * Seeds MAC212 (Media and Society) into the database.
 *
 * Uses the confirmed live department ID for Mass Communication, pulled
 * directly from crosslist-courses.js:
 *
 *   MASS_COMM: 'cms7arynn000asgby18h13uty'
 *
 * MAC212 is owned by Mass Communication but is cross-listed into
 * Criminology and Security Studies (confirmed as a shared course).
 * After running this script, you MUST run `node crosslist-courses.js`
 * to link it into Criminology as well - do not skip this step.
 *
 * NOTE: this is the FIRST course seeded for Mass Communication's own
 * department, beyond the GST courses it already received automatically
 * from the faculty/department setup script.
 *
 * HOW TO RUN:
 *   cd C:\Users\HP\Desktop\ubora-world
 *   node prisma\seed-data\seed-mac212.js
 *   node crosslist-courses.js --dry-run
 *   node crosslist-courses.js
 *
 * Place all 4 MAC212_CBT_Bank_Module1-4.csv files in the same folder
...
const MASS_COMM_DEPARTMENT_ID = "cms7arynn000asgby18h13uty";
...
async function main() {
  console.log("=== MAC212 Seed Script: Starting ===\n");

  const department = await prisma.department.findUnique({
    where: { id: MASS_COMM_DEPARTMENT_ID },
  });
  if (!department) {
    console.error(`!!! STOPPING: Department with id '${MASS_COMM_DEPARTMENT_ID}' not found.`);
  ...
  let course = await prisma.course.findUnique({ where: { code: "MAC212" } });
  if (!course) {
    ...
    console.log("Created Course: MAC212 - Media and Society (200L, 2nd semester)");
  } else {
    console.log("Found existing Course: MAC212");
  }
  ...
  if (!questionBank) {
    ...
    console.log("Created new draft QuestionBank for MAC212");
  } else {
    console.log(`Found existing QuestionBank for MAC212 (status: ${questionBank.status}) - will add/update questions in it`);
  }

  const dir = __dirname;
  const csvFiles = fs
    .readdirSync(dir)
    .filter((f) => f.startsWith("MAC212_CBT_Bank_Module") && f.endsWith(".csv"));

  if (csvFiles.length === 0) {
    console.log("\nNo MAC212_CBT_Bank_Module*.csv files found next to this script.");
    await prisma.$disconnect();
    return;
  }
  ...
  console.log(`\nNEXT STEP: run 'node crosslist-courses.js' so MAC212 also appears`);
