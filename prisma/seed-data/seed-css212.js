 ...
 * seed-css212.js
 *
 * Seeds CSS212 (The Sociology of Punishment and Correction) into the
 * database.
 *
 ...
 *
 * This is the FOURTH 200-level course built for Criminology (after
 * CSS246, CSS244, and PCR274). Level is explicitly set to 200, semester
 * to second, confirmed directly.
 *
 * This course is owned directly by Criminology (same as CSS111/121/
 * 133/112/132/134/246/244) - it is not a service course from another
 * department.
 *
 * NOTE - this is the largest course guide processed to date (348 pages,
 * 7 modules, 31 units total). The question bank draws from all 7
 * modules, weighted toward the richest content, and is grounded
 * throughout in the actual source material - including real Nigerian
 * case law (Baye Johnson v. AG Lagos State, 2002) and named
 * contributions from the 1990 National Conference on Prisons Reform.
 *
 ...
 *   node prisma\seed-data\seed-css212.js
 *   node crosslist-courses.js --dry-run
 *   node crosslist-courses.js
 *
 * Place all 7 CSS212_CBT_Bank_Module1-7.csv files in the same folder
...
async function main() {
  console.log("=== CSS212 Seed Script: Starting ===\n");
  ...
  let course = await prisma.course.findUnique({ where: { code: "CSS212" } });
  if (!course) {
    course = await prisma.course.create({
      data: {
        code: "CSS212",
        title: "The Sociology of Punishment and Correction",
        departmentId: department.id,
        level: 200,
        semester: "second",
      },
    });
    console.log("Created Course: CSS212 - The Sociology of Punishment and Correction (200L, 2nd semester)");
  } else {
    console.log("Found existing Course: CSS212");
  }
  ...
  if (!questionBank) {
    ...
    console.log("Created new draft QuestionBank for CSS212");
  } else {
    console.log(`Found existing QuestionBank for CSS212 (status: ${questionBank.status}) - will add/update questions in it`);
  }

  const dir = __dirname;
  const csvFiles = fs
    .readdirSync(dir)
    .filter((f) => f.startsWith("CSS212_CBT_Bank_Module") && f.endsWith(".csv"));

  if (csvFiles.length === 0) {
    console.log("\nNo CSS212_CBT_Bank_Module*.csv files found next to this script.");
    await prisma.$disconnect();
    return;
  }
  ...
  console.log(`\nREMINDER: run 'node crosslist-courses.js' now, even though CSS212 is`);
