 ...
 * seed-gst107.js
 *
 * Seeds GST103 (Computer Fundamentals) into the database:
 ...
 * used in seed-css111.js), then reads all GST103_CBT_Bank_Module*.csv
 ...
 *
 * Place the GST103_CBT_Bank_Module1-3.csv files (Modules 1-3, this
 * session) in the same folder as this script (prisma\seed-data\) before
 * running. Modules 4-6 will be added in a follow-up batch script once
 * drafted, matching the add-gst105-questions-batch2.js pattern used
 * for GST105's second batch.
 */
...
async function main() {
  console.log("=== GST103 Seed Script: Starting ===\n");

  let course = await prisma.course.findUnique({ where: { code: "GST103" } });
  if (!course) {
    ...
    console.log("Created Course: GST103 - Computer Fundamentals (GST, 100L, 1st semester)");
  } else {
    console.log("Found existing Course: GST103");
  }
  ...
  if (!questionBank) {
    ...
    console.log("Created new draft QuestionBank for GST103");
  } else {
    console.log(`Found existing QuestionBank for GST103 (status: ${questionBank.status}) — will add/update questions in it`);
  }

  const dir = __dirname;
  const csvFiles = fs
    .readdirSync(dir)
    .filter((f) => f.startsWith("GST103_CBT_Bank_Module") && f.endsWith(".csv"));

  if (csvFiles.length === 0) {
    console.log("\nNo GST103_CBT_Bank_Module*.csv files found next to this script.");
    await prisma.$disconnect();
    return;
  }
