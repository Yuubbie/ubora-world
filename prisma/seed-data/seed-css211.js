 ...
 * seed-css211.js
 *
 * Seeds CSS211 (The Sociology of Crime and Delinquency) into the
 * database.
 *
 ...
 *
 * This is the SECOND 200-level FIRST SEMESTER course built for
 * Criminology (alongside CSS241, seeded separately). Level is 200,
 * semester is "first", confirmed directly.
 *
 * This course is owned directly by Criminology (same as CSS111/121/
 * 133/112/132/134/246/244/212/241) - it is not a service course from
 * another department.
 *
 * This course is genuinely distinct in subject matter from every other
 * Criminology course built so far - it is classic sociological
 * criminology (Sutherland's Differential Association, Freud's
 * psychoanalytic theory, Merton's Anomie, Marxist conflict theory,
 * juvenile justice history) rather than security/threat-focused
 * content.
 *
 ...
 *   node prisma\seed-data\seed-css211.js
 *   node crosslist-courses.js --dry-run
 *   node crosslist-courses.js
 *
 * Place all 4 CSS211_CBT_Bank_Module1-4.csv files in the same folder
...
async function main() {
  console.log("=== CSS211 Seed Script: Starting ===\n");
  ...
  let course = await prisma.course.findUnique({ where: { code: "CSS211" } });
  if (!course) {
    course = await prisma.course.create({
      data: {
        code: "CSS211",
        title: "The Sociology of Crime and Delinquency",
        departmentId: department.id,
        level: 200,
        semester: "first",
      },
    });
    console.log("Created Course: CSS211 - The Sociology of Crime and Delinquency (200L, 1st semester)");
  } else {
    console.log("Found existing Course: CSS211");
  }
  ...
  if (!questionBank) {
    ...
    console.log("Created new draft QuestionBank for CSS211");
  } else {
    console.log(`Found existing QuestionBank for CSS211 (status: ${questionBank.status}) - will add/update questions in it`);
  }

  const dir = __dirname;
  const csvFiles = fs
    .readdirSync(dir)
    .filter((f) => f.startsWith("CSS211_CBT_Bank_Module") && f.endsWith(".csv"));

  if (csvFiles.length === 0) {
    console.log("\nNo CSS211_CBT_Bank_Module*.csv files found next to this script.");
    await prisma.$disconnect();
    return;
  }
  ...
  console.log(`\nREMINDER: run 'node crosslist-courses.js' now, even though CSS211 is`);
