 ...
 * seed-css132.js
 *
 * Seeds CSS132 (Ethnography of Nigeria) into the database.
 *
 ...
 *
 * NOTE - unlike every other course seeded so far, this course's official
 * NOUN course guide has NO Module/Unit split - it is structured as 11
 * units directly (Unit 1 through Unit 11), with no higher grouping.
 * To fit the database's `module` field on Question, each unit has been
 * mapped one-to-one to a module number (Unit 1 -> module 1, Unit 2 ->
 * module 2, ... Unit 11 -> module 11). There are therefore 11 CSV files,
 * not the usual 4-7.
 *
 * This course is OWNED directly by Criminology (same as CSS111/121/133/112)
 * - it is not a service course from another department.
 *
 ...
 * Criminology. Skipping that step was the mistake made with CSS112 -
 * the course will not show up in the app without it.
 *
 ...
 *   node prisma\seed-data\seed-css132.js
 *   node crosslist-courses.js --dry-run
 *   node crosslist-courses.js
 *
 * Place all 11 CSS132_CBT_Bank_Module1-11.csv files in the same folder
...
async function main() {
  console.log("=== CSS132 Seed Script: Starting ===\n");
  ...
  let course = await prisma.course.findUnique({ where: { code: "CSS132" } });
  if (!course) {
    course = await prisma.course.create({
      data: {
        code: "CSS132",
        title: "Ethnography of Nigeria",
        departmentId: department.id,
        level: 100,
        semester: "second",
      },
    });
    console.log("Created Course: CSS132 - Ethnography of Nigeria (100L, 2nd semester)");
  } else {
    console.log("Found existing Course: CSS132");
  }
  ...
  if (!questionBank) {
    ...
    console.log("Created new draft QuestionBank for CSS132");
  } else {
    console.log(`Found existing QuestionBank for CSS132 (status: ${questionBank.status}) - will add/update questions in it`);
  }

  const dir = __dirname;
  const csvFiles = fs
    .readdirSync(dir)
    .filter((f) => f.startsWith("CSS132_CBT_Bank_Module") && f.endsWith(".csv"));

  if (csvFiles.length === 0) {
    console.log("\nNo CSS132_CBT_Bank_Module*.csv files found next to this script.");
    await prisma.$disconnect();
    return;
  }
  ...
  console.log(`\nREMINDER: run 'node crosslist-courses.js' now, even though CSS132 is`);
