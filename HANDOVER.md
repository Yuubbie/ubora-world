...
- First semester Criminology (100L): **10 of 10 courses live and approved**
- Second semester Criminology (100L): **7 of 8 courses live and approved** (CSS136 outstanding)
- 200L Criminology: **1 course built (CSS246)** — the very start of the second-year build-out
...
### 200 Level — 1 course built (the start of the 200L build-out)
| Code | Title | Level | Semester | Owner Dept | Status |
|---|---|---|---|---|---|
| CSS246 | Legal and Social Framework of Private Security Services in Nigeria | 200 | second | Criminology | live |

Each completed course has: a CBT question bank (draft to approved), a downloadable PDF summary (draft to approved), and, where owned by another department, a `CourseDepartment` cross-listing row into Criminology.

**Note on CSS246:** the official NOUN course guide lists 20 units across 4 modules (5 per module), but Module 4's Unit 3 ("Relationship between Police and Private Security Operatives in Nigeria") had no extractable body text in the source PDF, only its title appeared in the table of contents. The question bank draws from the other 19 units, all genuinely grounded in the source material.
...
**Important note on GST auto-coverage:** GST courses use `isGST: true` with no `departmentId`, so they appear automatically for every department that exists, no manual linking needed. All 39 departments above already show GST101/102/103/105/107 to their students, even though only Criminology has any department-specific courses built so far (through 200L now, with CSS246).

**Structural fix made 3 Sep 2026:** the original "Computer Science" department (holding CIT104 and the demo course CSC103, plus 18 real recorded student attempts) was originally mis-attached to "Faculty of Sciences". This was corrected, the department was re-parented to the new "Faculty of Computing" without touching its id, its courses, or any student data. Confirmed with Danielson: "Maths and Computer Science" (a distinct joint-degree program) correctly stays under Faculty of Sciences and was not part of that move.
...
```
Faculty of Computing, Faculty of Management Sciences, and Faculty of Education were created 3 Sep 2026, look up their ids via `prisma.faculty.findUnique({ where: { name: '...' } })` if needed.

---

## 4. Key Scripts (all in project root or `prisma/seed-data/`)

- **`crosslist-courses.js`** — backfills each course's own-department link, plus cross-lists service courses (POL111, ECO121, PCR111, CIT104, POL126, PCR114 to Criminology) into Criminology. **Must be run after seeding any new course**, even ones owned directly by Criminology, this is not optional, it's how courses actually become visible in the app. Always supports `--dry-run`. **Whenever a new cross-listed service course is added, this file must be edited to add it to the `CROSS_LISTINGS` array**, the whole file gets replaced each time, so always grab the latest version from the most recent course delivered.
- **`setup-faculties-computing-sciences-mgmt-education.js`** — one-time setup script (already run 3 Sep 2026) that created the 3 new faculties, re-parented Computer Science, and created all 39 departments' Department rows. Safe to re-run (idempotent) if ever needed.
...
- Subscription gating (`lib/access.ts`) checks `endDate` live on every request, not a possibly-stale `status` field, so access is correctly revoked even if a background expiry job hasn't run yet, verified 4 Sep 2026, this was a deliberate design decision (see the "Master Spec 6.1 rule 3" comment in that file)

## 6. Env Vars Required
`DATABASE_URL`, `NEXTAUTH_SECRET`, `NEXTAUTH_URL`, `PAYSTACK_SECRET_KEY`

**Security note:** the live `DATABASE_URL` (Neon Postgres) and a Paystack key were pasted into a chat session on 2 Sep 2026. Recommended: rotate the Neon database password and regenerate the Paystack key from their respective dashboards, even though the Paystack key was test-mode. As of this update, there is no confirmation this rotation has happened yet.
...
1. **CSS136** — last course needed to complete second-semester 100L Criminology (waiting on Eunice)
2. **Continue building 200L Criminology** — CSS246 is the first of an unknown total; more course PDFs needed
...
8. **Rotate the Neon database password and Paystack key** (exposed in chat 2 Sep 2026, not yet confirmed rotated)
9. **Build department-specific course content for the four newer faculties** (Computing, Sciences, Management Sciences, Education) — currently they only have GST courses
...
- **2 Sep 2026:** ECO121 and PCR111 built and cross-listed, completing first-semester Criminology (10/10). CIT104 built for Computer Science, cross-listed into Criminology.
...
- **3 Sep 2026:** POL126 and PCR114 built and cross-listed, bringing second-semester Criminology to 7/8. Faculty of Computing, Faculty of Management Sciences, and Faculty of Education created; 24 new departments added; Computer Science department corrected from Faculty of Sciences to Faculty of Computing. Team briefed; pilot declared ready. README.md and this document brought up to date; local git history for all seed scripts and CSVs committed and pushed for the first time (previously local-only).
- **4 Sep 2026:** Confirmed via direct code review that the subscription-gating logic (`lib/access.ts`) is correctly implemented, checks live expiry, not just tier, closing any doubt about non-subscribers accessing gated summaries. CSS246 built, the first 200-level Criminology course, marking the start of the 200L build-out alongside the still-incomplete second-semester 100L set.
