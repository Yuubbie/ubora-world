// add-css132-summary.js
// Creates the Summary record for CSS132 (Ethnography of Nigeria).
//
...
//        C:\Users\HP\Desktop\ubora-world\private-uploads\css132-summary.pdf
//   2. Confirm CSS132 has already been seeded (seed-css132.js) AND that
//      you have already run crosslist-courses.js after seeding it.
//
// Run from the project root:
//   node add-css132-summary.js
...
const PDF_PATH = path.join(process.cwd(), 'private-uploads', 'css132-summary.pdf');

async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'CSS132' } });
  if (!course) {
    console.error('CSS132 course not found. Run seed-css132.js first.');
  ...
  if (existing) {
    console.log('A summary already exists for CSS132:', existing.id);
  ...
  if (!fs.existsSync(PDF_PATH)) {
    ...
    console.error('Place css132-summary.pdf in the private-uploads folder and re-run.');
    await prisma.$disconnect();
    process.exit(1);
  }
  ...
  const summary = await prisma.summary.create({
    data: {
      courseId: course.id,
      title: 'CSS132 Course Summary - Ethnography of Nigeria',
      topicCount: 11,
      pageCount: 5,
      fileUrl: 'db-stored:css132-summary',
      fileData: buffer,
      status: 'draft',
    },
  });
