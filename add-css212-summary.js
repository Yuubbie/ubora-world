// add-css212-summary.js
// Creates the Summary record for CSS212 (The Sociology of Punishment
// and Correction).
//
...
//        C:\Users\HP\Desktop\ubora-world\private-uploads\css212-summary.pdf
//   2. Confirm CSS212 has already been seeded (seed-css212.js) AND that
//      you have already run crosslist-courses.js after seeding it.
//
// Run from the project root:
//   node add-css212-summary.js
...
const PDF_PATH = path.join(process.cwd(), 'private-uploads', 'css212-summary.pdf');

async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'CSS212' } });
  if (!course) {
    console.error('CSS212 course not found. Run seed-css212.js first.');
  ...
  if (existing) {
    console.log('A summary already exists for CSS212:', existing.id);
  ...
  if (!fs.existsSync(PDF_PATH)) {
    ...
    console.error('Place css212-summary.pdf in the private-uploads folder and re-run.');
    await prisma.$disconnect();
    process.exit(1);
  }
  ...
  const summary = await prisma.summary.create({
    data: {
      courseId: course.id,
      title: 'CSS212 Course Summary - The Sociology of Punishment and Correction',
      topicCount: 31,
      pageCount: 8,
      fileUrl: 'db-stored:css212-summary',
      fileData: buffer,
      status: 'draft',
    },
  });
